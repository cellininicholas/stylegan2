...
def generate_latent_images(zs, truncation_psi, save_npy, prefix):
    ...
    for z_idx, z in enumerate(zs):
        ...
        PIL.Image.fromarray(images[0], 'RGB').save(
            dnnlib.make_run_dir_path('%s%05d.png' % (prefix, z_idx)))
        if save_npy:
            np.save(dnnlib.make_run_dir_path(
                '%s%05d.npy' % (prefix, z_idx)), z)
...
def generate_neighbors(network_pkl, seeds, npys, diameter, truncation_psi, num_samples, save_vector):
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        tflib.set_vars({var: rnd.randn(*var.shape.as_list())
                        for var in noise_vars})  # [height, width]
        # [minibatch, height, width, channel]
        images = Gs.run(og_z, None, **Gs_kwargs)
        PIL.Image.fromarray(images[0], 'RGB').save(
            dnnlib.make_run_dir_path('seed%04d.png' % seed))
...
def get_latent_interpolation_bspline(endpoints, nf, k, s, shuffle):
    ...
    for i in tqdm(range(nd-9)):
        ...
        tck, u = interpolate.splprep([x[:, j]
                                      for j in range(i, i+10)], k=k, s=s)
...
def generate_latent_walk(network_pkl, truncation_psi, walk_type, frames, seeds, npys, save_vector, diameter=2.0, start_seed=0):
    ...
    # compute points in latent spaces
    if walk_type[0] == 'line':
    ...
    # from Gene Kogan
        ...
        points = get_latent_interpolation_bspline(
            z, frames, 3, 20, shuffle=False)
    ...
    elif walk_type[0] == 'square':
        points = square_interpolate(zs, frames)

    if (walk_type[0] == 'line' and len(walk_type) > 1 and walk_type[1] == 'w'):
        generate_images_in_w_space(
            points, truncation_psi, save_vector, 'frame')
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    parser_truncation_traversal = subparsers.add_parser(
        'truncation-traversal', help='Generate truncation walk')
    parser_truncation_traversal.add_argument(
        '--network', help='Network pickle filename', dest='network_pkl', required=True)
    parser_truncation_traversal.add_argument(
        '--seed', type=_parse_num_range, help='Singular seed value')
    parser_truncation_traversal.add_argument(
        '--npys', type=_parse_npy_files, help='List of .npy files')
    parser_truncation_traversal.add_argument(
        '--start', type=float, help='Starting value')
    parser_truncation_traversal.add_argument(
        '--stop', type=float, help='Stopping value')
    parser_truncation_traversal.add_argument(
        '--increment', type=float, help='Incrementing value')
    parser_truncation_traversal.add_argument(
        '--result-dir', help='Root directory for run results (default: %(default)s)', default='results', metavar='DIR')

    parser_generate_latent_walk = subparsers.add_parser(
        'generate-latent-walk', help='Generate latent walk')
    ...
    parser_generate_latent_walk.add_argument(
        '--diameter', type=float, help='diameter of noise loop', default=2.0)
    parser_generate_latent_walk.add_argument(
        '--start_seed', type=int, help='random seed to start noise loop from', default=0)
    parser_generate_latent_walk.add_argument(
        '--result-dir', help='Root directory for run results (default: %(default)s)', default='results', metavar='DIR')

    parser_generate_neighbors = subparsers.add_parser(
        'generate-neighbors', help='Generate random neighbors of a seed')
    parser_generate_neighbors.add_argument(
        '--network', help='Network pickle filename', dest='network_pkl', required=True)
    parser_generate_neighbors.add_argument(
        '--seeds', type=_parse_num_range, help='List of random seeds')
    parser_generate_neighbors.add_argument(
        '--npys', type=_parse_npy_files, help='List of .npy files')
    parser_generate_neighbors.add_argument(
        '--diameter', type=float, help='distance around seed to sample from', default=0.1)
    parser_generate_neighbors.add_argument(
        '--save_vector', dest='save_vector', action='store_true', help='also save vector in .npy format')
    parser_generate_neighbors.add_argument(
        '--num_samples', type=int, help='How many neighbors to generate (default: %(default)s', default=25)
    parser_generate_neighbors.add_argument(
        '--truncation-psi', type=float, help='Truncation psi (default: %(default)s)', default=0.5)
    parser_generate_neighbors.add_argument(
        '--result-dir', help='Root directory for run results (default: %(default)s)', default='results', metavar='DIR')
    ...
    parser_generate_images.add_argument(
        '--seeds', type=_parse_num_range, help='List of random seeds')
    parser_generate_images.add_argument(
        '--npys', help='List of .npy files', dest='npy_files')
    parser_generate_images.add_argument(
        '--truncation-psi', type=float, help='Truncation psi (default: %(default)s)', default=0.5)
    parser_generate_images.add_argument(
        '--result-dir', help='Root directory for run results (default: %(default)s)', default='results', metavar='DIR')
